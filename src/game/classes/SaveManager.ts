...
export class SaveManager {
    ...
    static loadGame(slotName: string, gridState: GridState, player: Player, plantManager: PlantManager, zombies: any[], scene: DefaultScene): GameState | null {
        const serializedState = localStorage.getItem(`save_${slotName}`);
        if (serializedState) {
            return GameState.deserialize(serializedState, gridState, player, plantManager, zombies, scene);
        }
    ...
    static getSaveSlots(): string[] {
        return Object.keys(localStorage).filter(key => key.startsWith('save_')).map(key => key.replace('save_', ''));
    }
